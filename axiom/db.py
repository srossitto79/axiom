"""SQLite database — single source of truth for all Axiom state."""
...
log = logging.getLogger("axiom.db")
...
_FACTORY_RESET_KV_KEYS = {
    ...
    "secrets": (
        "axiom:settings:secrets",
    ),
    "api-keys": (
        "axiom:settings:api-keys",
    ),
}
...
def backup_db(destination: str | Path) -> Path:
    ...
    src = sqlite3.connect(str(AXIOM_DB), timeout=30)
    try:
...
def init_db():
    ...
    with get_db() as conn:
        ...
        # any new-style migrations registered in Axiom.migrations.
        conn.execute("SAVEPOINT named_migrations")
        try:
            from axiom.migrations import apply_pending, record_applied
...
def recover_dangling_runtime_tasks() -> dict[str, int]:
    ...
    """
    from axiom.system_mode_policy import initial_queue_status_for_source, normalize_task_source
...
def _run_migrations(conn: sqlite3.Connection):
    ...
    #
    # mcp_servers — config for each external MCP server Axiom can connect to
...
def _run_post_index_migrations(conn: sqlite3.Connection):
    ...
    """
    # Speeds existing trade source filters, including defensive exclusion of
    # historical source='bot:*' rows from live risk views.
    try:
...
def _ensure_column(conn: sqlite3.Connection, table: str, column: str, col_type: str):
    ...
    doesn't exist yet. Some tables live in POST_MIGRATION_INDEXES_SQL which runs
    AFTER _run_migrations; on fresh DBs those tables land later with their full
    modern schema already containing the columns these migrations would add, so
...
def kv_get(key: str, default=None):
    ...
    if row:
        ...
        if isinstance(raw_value, (str, bytes, bytearray)):
            ...
            payload = raw_value
        if key == "axiom:settings:secrets" and isinstance(payload, dict):
            try:
...
_USER_ACTIVE_KEY = "axiom:user_active"
...
def log_activity(
    ...
    try:
        from axiom.correlation import get_request_id
        rid = get_request_id()
    ...
    try:
        ...
        if _is_sqlite_lock_error(exc):
            logger = logging.getLogger("axiom.db")
            logger.warning(
...
def get_open_trades(exclude_bots: bool = False) -> list[dict]:
    ...
    When ``exclude_bots`` is True, legacy bot-tagged paper trades
    (source='bot:{id}') are omitted so live/strategy risk-reasoning contexts
    never count them as real exposure. Defaults False to preserve every
    existing caller.
    """
...
def create_approval(
    ...
    expires_at = None
    try:
        from axiom.control_plane.approval_modes import get_deadline_hours
        from datetime import datetime, timedelta, timezone
    ...
    if conn is None:
        try:
            from axiom.control_plane.approval_modes import (
                get_mode,
                is_off_allowed,
            )
            ...
            if mode == "smart":
                from axiom.control_plane.smart_approval import apply_smart_decision
...
def reap_long_running_agent_tasks(timeout_minutes: int = 30) -> int:
    ...
    from axiom.task_timeouts import REAPER_GRACE_MINUTES, resolve_agent_task_timeout_seconds
...
def _factory_reset_wipe_chroma_collections(collections: list[str]) -> None:
    if not collections:
        return
    try:
        from axiom.vectordb import wipe_collections
...
def factory_reset(keep_categories: list[str] | None = None, *, allow_credentials_wipe: bool = False) -> dict:
    ...
    if "scheduler_jobs" in wipe_set:
        from axiom.scheduler import seed_AXIOM_jobs

        seed_AXIOM_jobs()

    if "system_docs" in wipe_set:
        from axiom.workspace import _create_defaults
        ...
        with get_db() as conn:
            create_pending_task(
                ...
                "brain_invoke",
                {
                    ...
                    "message": (
                        "axiom was just factory-reset. You are the Brain — the sole orchestrator.\n\n"
...
def mark_backtest_failed(strategy_id: str, failure_type: str, reason: str) -> None:
    ...
    try:
        from axiom.vectordb import store_post_mortem
        store_post_mortem(
...
def verify_chroma_persistence(result_id: str) -> tuple[bool, str]:
    ...
    try:
        ...
        from axiom.config import CHROMA_DIR
