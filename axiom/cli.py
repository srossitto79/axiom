...
@click.group()
@click.version_option(package_name="axiom")
def cli():
...
def auth_status():
    ...
    from axiom.auth.store import display_status
    display_status()
...
def auth_login(provider):
    ...
    from axiom.auth.store import run_login
    run_login(provider)
...
def auth_refresh(provider):
    ...
    from axiom.auth.store import force_refresh
...
def auth_migrate():
    ...
    from axiom.auth.store import migrate_from_openclaw
    migrate_from_openclaw()


@auth.command("init-operator-key")
def auth_init_operator_key():
    """Generate and print AXIOM_OPERATOR_KEY for .env configuration."""
    import secrets
    import os
    ...
    print(f"  export AXIOM_OPERATOR_KEY='{key}'")
    print(f"  # or in .env:")
    print(f"  AXIOM_OPERATOR_KEY={key}\n")
    print("Then restart the Axiom backend:\n")
    print("  python -m axiom api\n")
...
def configure():
    ...
    from axiom.auth.store import interactive_configure
...
def ai_ask(provider, prompt, model):
    ...
    from axiom.ai import call_ai_sync
    from axiom.model_selection import ensure_enforcement_armed
...
def db_migrate():
    ...
    from axiom.config import OPENCLAW_WORKSPACE
    from axiom.db import init_db, migrate_from_openclaw
...
def db_restore_snapshot(snapshot_id):
    ...
    from axiom.db import init_db, restore_migration_snapshot
...
def db_status():
    ...
    from axiom.db import init_db, table_counts
...
def workspace_list():
    ...
    from axiom.workspace import list_workspace_files
...
def trades_list(limit):
    ...
    from rich.console import Console
    from rich.table import Table
    from axiom.db import init_db, get_recent_trades
...
def trades_open():
    ...
    from rich.console import Console
    from rich.table import Table
    from axiom.db import init_db, get_open_trades
...
def strategies_list(status):
    ...
    from rich.console import Console
    from rich.table import Table
    from axiom.db import init_db, get_strategies
...
def bot_send(channel, message):
    ...
    import asyncio
    from axiom.bot import send, get_bot, get_bot_token
...
def scheduler_list():
    ...
    from rich.table import Table
    from axiom.db import init_db
    from axiom.scheduler import get_jobs

    init_db()
    jobs = get_jobs()
    if not jobs:
        click.echo("No scheduler jobs. Run `Axiom scheduler migrate` to import from OpenClaw.")
        return
...
def scheduler_migrate():
    ...
    from axiom.scheduler import migrate_from_openclaw
    migrate_from_openclaw()
...
def scheduler_run(job_id):
    ...
    with __import__("axiom.db", fromlist=["get_db"]).get_db() as conn:
...
def scanner_status():
    ...
    from axiom.db import init_db, kv_get
    init_db()
    state = kv_get("scanner_state", {})
    if not state:
        click.echo("No scanner state found. Run `Axiom scanner run` first.")
        return
...
def scanner_backtest(bars, strategy):
    ...
    from rich.console import Console
    from rich.table import Table
    from axiom.strategies.backtest import backtest_strategy, backtest_all, save_backtest_results
    from axiom.strategies.fitness import evaluate_all
    from axiom.scanner import STRATEGIES
...
def brain_invoke(provider, model):
    ...
    from axiom.brain import invoke_sync
    from axiom.model_selection import ensure_enforcement_armed
    ensure_enforcement_armed()
...
def risk_status():
    ...
    from axiom.db import init_db
    from axiom.exchange.risk import get_portfolio_summary, get_risk_status, PORTFOLIO_BUDGET
    ...
    if rs["kill_switch_active"]:
        ...
        click.echo("  All trading halted. Run `Axiom risk reset` to clear after review.")
...
def risk_reset():
    ...
    from axiom.db import init_db
    from axiom.exchange.risk import reset_kill_switch, get_risk_status
...
def team_list():
    ...
    from rich.table import Table
    from axiom.db import init_db
    from axiom.agents.manager import list_agents_with_stats

    init_db()
    agents = list_agents_with_stats()
    if not agents:
        click.echo("No agents configured. Create one with: Axiom team create")
        return
...
def team_inspect(agent_id):
    ...
    from axiom.agents.manager import inspect_agent
...
def team_enable(agent_id):
    """Enable an agent."""
    from axiom.agents.manager import update_agent
...
def evolution_status():
    ...
    from rich.console import Console
    from rich.table import Table
    from axiom.evolution import get_evolution_status
    from axiom.db import init_db
...
def evolution_optimize(strategy_id):
    ...
    from rich.console import Console
    from axiom.strategies.optimizer import optimize_strategy, optimize_all_deployed
    from axiom.db import init_db
...
def regime_status():
    ...
    from rich.console import Console
    from rich.table import Table
    from axiom.regime import detect_all_regimes
    from axiom.db import init_db
...
def lab_worker(once, poll_seconds, lease_seconds):
    ...
    from axiom.lab_worker_service import get_canonical_lab_worker_log_path, run_lab_worker_loop
...
def lab_status():
    ...
    from rich.table import Table
    from axiom.lab_db import init_lab_db
    from axiom.lab_worker_service import get_lab_worker_status
...
def execution_status():
    ...
    try:
        from axiom.exchange.hyperliquid import get_positions, get_account_value
...
def execution_reconcile():
    ...
    from rich.console import Console
    from axiom.exchange.risk import reconcile_all_books
    from axiom.db import init_db
...
def status():
    ...
    # Auth
    from axiom.auth.store import get_status_rows
    ...
    try:
        ...
        if any(c > 0 for c in counts.values()):
            ...
            console.print("[dim]Database: empty (run `Axiom db migrate` to import data)[/dim]")
    except Exception as e:
    ...
    if files:
        ...
        console.print("[dim]Workspace: not initialized (run `Axiom workspace init`)[/dim]")
...
def fts5_rebuild():
    ...
    """
    from axiom.db import init_db, rebuild_fts5_indices
...
def doctor(as_json):
    ...
    from axiom.diagnostics import FAIL, PASS, WARN, snapshot
    ...
    console.print(f"\n[bold {color}]Axiom doctor — overall: {overall.upper()}[/bold {color}]")
    console.print(
...
def market_data_backfill(asset, days):
    ...
    from axiom.db import init_db
    from axiom.market_data_collector import backfill_all, backfill_funding_history
...
def market_data_coverage(asset):
    ...
    from axiom.db import init_db
    from axiom.market_data_collector import get_data_coverage

    init_db()
    report = get_data_coverage(asset)
    if not report.get("coverage"):
        click.echo("No market data collected yet. Run: Axiom market-data backfill")
        return
...
def dump_cmd(out):
    ...
    from axiom.db import get_db
    from axiom.workspace import read_operator_profile
