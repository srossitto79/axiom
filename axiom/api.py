...
from axiom.api_core import AxiomV1CompatMiddleware, _on_startup
from axiom.api_security import (
    ApiKeyMiddleware,
...
from axiom.correlation import CorrelationIdMiddleware, RequestIdLogFilter
from axiom.rate_limiting import install_rate_limiter
from axiom.control_plane import (
    ConfirmBody,
    ExecutionModeBody,
    post_emergency_halt,
    post_execution_mode,
...
from axiom.routers.analytics import router as analytics_router
from axiom.routers.approvals import router as approvals_router
from axiom.routers.auth import router as auth_router
from axiom.routers.agents import router as agents_router
from axiom.routers.agent_toolsets import router as agent_toolsets_router
from axiom.routers.data import router as data_router
from axiom.routers.deepdive import router as deepdive_router
from axiom.routers.assistant import router as assistant_router
from axiom.routers.jobs import router as jobs_router
from axiom.routers.legacy import router as legacy_router
from axiom.routers.memory import router as memory_router
from axiom.routers.diagnostics import router as diagnostics_router
from axiom.routers.mcp import router as mcp_router
from axiom.routers.notifications import router as notifications_router
...
from axiom.routers.strategies import router as strategies_router
from axiom.routers.strategy_library import router as strategy_library_router
from axiom.routers.system import router as system_router
from axiom.routers.tasks import router as tasks_router
from axiom.routers.trading import router as trading_router
from axiom.routers.websockets import router as websockets_router
from axiom.routers.quant_factory import router as quant_factory_router
from axiom.routers.routines import router as routines_router
from axiom.routers.profile import router as profile_router
from axiom.routers.webhooks import router as webhooks_router
from axiom.routers.updates import router as updates_router
from axiom.routers.backtesting import router as backtesting_router
from axiom.routers.lifecycle import router as lifecycle_router
from axiom.routers.simulation import router as simulation_router, simulation_api_enabled
from axiom.routers.verdict import router as verdict_router
...
from axiom.routers.health import router as health_router
from axiom.routers.hypotheses import router as hypotheses_router, data_gap_router
...
def _spawn_supervised_runtime_thread(
    ...
    *,
    ...
    thread = threading.Thread(target=_runner, name=f"axiom-{name}", daemon=True)
    thread.start()
    return thread
...
if sys.platform.startswith("win"):
    ...
    # uvicorn.
    os.environ.setdefault("AXIOM_DISABLE_CHROMA_IN_PROCESS", "1")
...
def _configure_runtime_logging() -> None:
    ...
    production launcher (``python -m uvicorn axiom.api:app``) the ``__main__``
    ...
    try:
        if home_env:
            ...
            from axiom.logging_config import setup_rotating_file_logger

            log_path = _Path(home_env) / "logs" / "api.log"
            setup_rotating_file_logger(
                log_path, level=logging.INFO, fmt=fmt, also_stdout=True
            )
            logging.getLogger("axiom.api").info("Runtime logging -> %s", log_path)
...
async def lifespan(_app: FastAPI):
    ...
    # Packaged installs: seed $AXIOM_HOME/data/ohlcv/ from bundled parquets
    ...
    # returns empty. No-op for dev runs (AXIOM_HOME unset) and for repeat
    ...
    try:
        from axiom.config import ensure_seed_data_bootstrapped
    ...
    try:
        from axiom.lifecycle import resume_interrupted_tasks_on_startup
    ...
    # Seed core Axiom agents (brain, quant-researcher, simulation-agent,
    ...
    try:
        from axiom.model_selection import (
            enable_enforcement,
    ...
    try:
        from axiom.workspace import init_workspace
    ...
    try:
        from axiom.bot import seed_default_agents
    ...
    try:
        from axiom.agents.mcp_client import register_all_enabled_servers
    ...
    try:
        if acquire_runtime_worker_lock():
            ...
            from axiom.scheduler import reset_scheduler_job_locks, run_scheduler_loop
            ...
            agent_concurrency = _env_int("AXIOM_HEADLESS_AGENT_CONCURRENCY", 3, minimum=1, maximum=8)
            brain_limit = _env_int("AXIOM_HEADLESS_BRAIN_LIMIT", 2, minimum=1, maximum=8)
            runtime_thread_mode = _env_bool("AXIOM_API_RUNTIME_THREAD_MODE", True)
            runtime_start_delay = _env_float("AXIOM_API_RUNTIME_START_DELAY_SECONDS", 5.0, minimum=0.0, maximum=60.0)
            ...
            # (Tauri shell only spawns `python -m axiom.api`) still drives the
            # market feed, price ticks, and daemon_state heartbeat. The daemon
            # holds its own file lock so an external `Axiom daemon start` wins
            # if it's already running.
            try:
                from axiom.daemon import run_in_loop as run_daemon_in_loop
                if runtime_thread_mode:
    ...
    try:
        from axiom.health_monitor import HealthMonitor, HealthState, set_health_monitor
    ...
    try:
        from axiom.daemon import shutdown as _daemon_shutdown_event
...
app.add_middleware(AxiomV1CompatMiddleware)
...
# closes that door. AXIOM_ALLOWED_HOSTS can extend the list for dev/LAN
# scenarios; packaged builds stick to loopback.
_allowed_hosts_env = os.environ.get("AXIOM_ALLOWED_HOSTS", "").strip()
...
from axiom.config import is_beta_build as _is_beta_build  # noqa: E402
...
# Serve the prebuilt SvelteKit bundle at "/" when AXIOM_FRONTEND_DIR points to
...
_frontend_dir = os.environ.get("AXIOM_FRONTEND_DIR")
...
if __name__ == "__main__":
    ...
    # Packaged installs (Tauri sets AXIOM_HOME) have no attached console — the
    ...
    # actually read. Dev runs (no AXIOM_HOME) keep the existing stdout-only
    # behavior so `Axiom api` in a terminal stays chatty.
    # Same logging bootstrap the lifespan uses; idempotent via the module guard.
    # Running it here too means a direct `python -m axiom.api` launch still
    # configures logging before uvicorn.run() (so bind/startup errors are
    # captured), while `uvicorn axiom.api:app` relies on the lifespan call.
    _configure_runtime_logging()

    parser = argparse.ArgumentParser()
    parser.add_argument("--port", type=int, default=None)
    args = parser.parse_args()
    port = args.port if args.port is not None else int(os.environ.get("AXIOM_PORT", "8003"))
    ...
    # AXIOM_BIND_HOST (or the AXIOM_HOST launcher alias) can be set for dev/ops
    ...
    from axiom.api_security import resolved_bind_host
