...
from axiom.binance_vision import bv_client

log = logging.getLogger("axiom.data_manager")
...
# AXIOM_HOME in packaged installs). Previously _BASE_DIR hardcoded a repo-relative
...
from axiom.data import data_root as _data_root
...
def assert_data_root_consistent() -> bool:
    ...
    if ohlcv_root != stream_root:
        log.warning(
            ...
            "on zeros. Align AXIOM_DATA_DIR / AXIOM_HOME.",
            ohlcv_root,
            stream_root,
        )
        return False
...
def _save_stream_parquet(df: pd.DataFrame, path: Path, stream: str, symbol: str) -> None:
    """Atomic write of a stream DataFrame to parquet with Axiom metadata.
    ...
    try:
        ...
        meta.update({
            b"AXIOM_source": b"binanceusdm",
            b"AXIOM_stream": stream.encode(),
            b"AXIOM_symbol": symbol.encode(),
            b"AXIOM_updated_at": _now_iso().encode(),
        })
...
def _fetch_stream_via_source_registry(
    ...
    *,
    ...
    from axiom.dataeng.errors import NoData
    from axiom.dataeng.identity import to_ref
    from axiom.dataeng.source import Stream, get_source_registry
...
class FundingCollector:
    ...
    def collect(self, symbol: str) -> int:
        ...
        from axiom.data import symbol_to_fs, symbol_to_ccxt
...
class _RestCollector:
    ...
    def collect(self, symbol: str) -> int:
        from axiom.data import symbol_to_fs
        fs_symbol = symbol_to_fs(symbol)
...
class DataManager:
    ...
    def _normalize_keepalive_symbol(self, symbol: str | None, *, require_dataset: bool = False) -> str | None:
        ...
        """
        from axiom.data import DATA_DIR, symbol_to_fs
    ...
    def collect_liquidations(self) -> dict[str, Any]:
        ...
        auth-gated/deprecated. Set AXIOM_ENABLE_LIQUIDATIONS=1 to opt in."""
        # Env-gated early return: do NOT record a run when disabled (T22).
        if os.environ.get("AXIOM_ENABLE_LIQUIDATIONS") != "1":
    ...
    def _backfill_ohlcv(self, fs_sym: str, bv_symbol: str) -> dict:
        from axiom.data import DATA_DIR, load_parquet, save_parquet, _get_dataset_lock
        out: dict = {}
    ...
    def _apply_lan_enrichment(
        ...
        *,
        ...
        try:
            from axiom.lan_enricher import get_lan_enricher
...
# Existing callers — `from axiom.data_manager import data_manager` and
