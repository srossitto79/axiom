...
from axiom.codex_responses import is_openai_oauth_token
from axiom.model_routing import (
    get_default_model_for_provider,
    get_model_routing_snapshot,
    update_model_routing,
)
from axiom.config import AUTH_FILE, AXIOM_HOME, is_beta_build
from axiom.agents.manager import create_agent, delete_agent, inspect_agent, update_agent
from axiom.auth.store import (
    delete_profile,
    get_profile,
    get_token,
    is_profile_opaque,
...
from axiom.secret_storage import decrypt_secret, encrypt_secret
from axiom import strategy_lifecycle as lifecycle_service
from axiom.workspace import read_workspace, write_workspace
from axiom.util import generate_pkce, generate_state, normalize_stage

log = logging.getLogger("axiom.api")
_BACKTEST_RESULTS_REMOTE_API_ENV = "AXIOM_BACKTEST_RESULTS_REMOTE_API"
...
def _optimization_executor_workers() -> int:
    raw = str(os.environ.get("AXIOM_OPTIMIZATION_MAX_WORKERS", "2") or "").strip()
    try:
        parsed = int(raw)
    except Exception:
        parsed = 2
...
class AxiomV1CompatMiddleware(BaseHTTPMiddleware):
	"""Allow frontend calls that use `/api/Axiom/*` or `/api/axiom/*`."""

	async def dispatch(self, request, call_next):
		path = request.scope.get("path", "")
		legacy_path = None
		if path.startswith("/api/Axiom/") or path.startswith("/api/axiom/"):
			legacy_path = path
			request.scope["path"] = "/api/" + path.split("/", 3)[3]
		elif path == "/api/Axiom" or path == "/api/axiom":
			legacy_path = path
		...
		if legacy_path:
			...
			response.headers.setdefault("X-Axiom-Legacy-Route", legacy_path)
		return response
...
async def _on_startup():
    ...
    # Check if AXIOM_OPERATOR_KEY is configured
    if not os.getenv("AXIOM_OPERATOR_KEY"):
        log.warning(
            "AXIOM_OPERATOR_KEY not set. This is required to configure LLM providers. "
            "Run: python -m axiom auth init-operator-key"
        )

    _BOOTSTRAP_MAX_RETRIES = 3
    _BOOTSTRAP_RETRY_DELAY = 5.0
    try:
        from axiom.db import recover_dangling_runtime_tasks
        from axiom.system_mode_policy import reconcile_manual_mode_backlog
    ...
    try:
        from axiom.data_manager import assert_data_root_consistent
        ...
        # AXIOM_DATA_DIR / AXIOM_HOME are aligned.
        if not assert_data_root_consistent():
            log.error(
                ...
                "UNRELIABLE until AXIOM_DATA_DIR / AXIOM_HOME are aligned "
...
def _discover_provider_models(provider: str, force_refresh: bool = False) -> tuple[list[dict], str | None]:
    ...
    if provider == "openrouter":
        ...
        # useful set for Axiom's agent loop), auto-updating as the free roster
...
def _build_auth_provider_payload(provider: str) -> dict:
    ...
    refresh_command = (
        ...
        if provider == "lmstudio"
        else f"axiom auth refresh {provider}"
    )
...
def _get_auth_providers_compat() -> dict:
    return {
        ...
        "configure_command": "axiom auth status",
        "status_command": "axiom auth status",
...
def _resolve_exchange_testnet() -> bool:
    from axiom.api_domains.trading import _resolve_exchange_testnet as _domain_resolve_exchange_testnet
...
_SETTINGS_STORAGE_KEY = "axiom:settings"
_SETTINGS_SECRET_STORAGE_KEY = "axiom:settings:secrets"
_SETTINGS_API_KEYS_STORAGE_KEY = "axiom:settings:api-keys"
_SETTINGS_PIPELINE_STORAGE_KEY = "axiom:pipeline:settings"
...
_DEFAULT_SETTINGS_PAYLOAD = {
    ...
    # Strict mode for the agent run_shell command guard (Axiom.sandbox.shell_guard).
...
# Only Polygon is wired to a consumer (Axiom.config.get_polygon_api_key ->
...
_DEFAULT_PIPELINE_SETTINGS = {
    ...
    # Consumed by Axiom.maintenance.run_db_maintenance via this same payload.
    ...
    # Aligned with the canonical gate store (Axiom:pipeline_thresholds /
...
def _default_data_engine_settings_payload() -> dict:
    from axiom.dataeng.settings import default_data_engine_settings_payload

    return default_data_engine_settings_payload()


def _merge_data_engine_settings_payload(value) -> dict:
    from axiom.dataeng.settings import merge_data_engine_settings_payload
...
def _default_research_settings_payload() -> dict:
    from axiom.research_contract import default_research_settings
...
def _apply_settings_section(section: str, payload: dict) -> dict:
    ...
    if section == "exchange":
        ...
        if exchange:
            ...
            try:
                from axiom.exchange.sync_wrapper import reset_sync_exchange
                reset_sync_exchange()
            except Exception:
                pass
        ...
        if "trading_mode" in payload:
            ...
            try:
                from axiom.config import set_execution_mode
        ...
        # Promotion-safety gates (read top-level from Axiom:settings by
        # Axiom.policy.evaluate_promotion and Axiom.hypothesis_graduation).
        ...
        # The live trade gate (Axiom.regime.is_strategy_allowed -> Axiom.config
        ...
        # (Axiom:notification_preferences), which resolve_notification_policy
    ...
    try:
        import os as _os
        from axiom.config import load_config as _load_cfg
        _cfg = _load_cfg()
    ...
    if section == "hyperliquid" and bool(updates.get("hyperliquid_has_key")):
        try:
            from axiom.exchange.hyperliquid import _get_creds
            _get_creds()
...
def _build_openai_oauth_start() -> dict[str, object]:
    from axiom.auth import openai as openai_auth
...
def _build_minimax_oauth_start() -> dict[str, str]:
    from axiom.auth import minimax as minimax_auth
...
def _result_data_dirs() -> list[str]:
    ...
    candidates = [
        ...
        os.path.join(str(AXIOM_HOME), "data", "results"),
    ]
    out: list[str] = []
    seen: set[str] = set()
...
def _chroma_backtest_records():
    ...
    from axiom.vectordb import _check_chroma_available
...
async def post_brain_chat_direct(body: BrainChatBody):
    ...
    from axiom.context import build_chat_context, store_conversation
    ...
    try:
        ...
        from axiom.ai import _is_rate_limit_exception
...
# blob (default 30, enforced by Axiom.exchange.risk reading the blob directly)
...
def get_settings():
    ...
    try:
        ...
        pass
    try:
        from axiom.policy import load_pipeline_config, pipeline_thresholds_for_display
    ...
    try:
        from axiom.policy import _normalize_pipeline_config, pipeline_thresholds_for_display
    ...
    try:
        from axiom import config as _regime_cfg
    ...
    try:
        from axiom.notifications import get_notification_preferences
...
def get_settings_discord_audit(send_probe: bool = False):
    try:
        from axiom.bot import run_discord_audit
...
def update_pipeline_config(config: dict):
    ...
    from axiom.policy import save_pipeline_config
...
def upsert_auth_provider(provider: str, body: AuthProviderProfileBody):
    ...
    try:
        from axiom.model_selection import mark_provider_connected
...
def delete_auth_provider(provider: str):
    ...
    try:
        from axiom.model_selection import unmark_provider_connected
...
def post_agent_test_discord(agent_id: str, payload: AgentDiscordTestBody | None = None):
    ...
    if not token:
        try:
            from axiom.bot import get_bot_token
    ...
    from axiom.bot import CHANNELS
    from axiom.reporter import AGENT_CHANNEL_MAP
    ...
    tested_at = _now()
    message = (
        f"[Axiom Settings] Gateway Discord test message for {agent_name} ({normalized_agent_id}) at {tested_at}.\n"
...
def update_strategy_default_params(
    ...
    *,
    ...
    from axiom.brain import update_strategy_params
    ...
    try:
        from axiom.db import append_strategy_event
        ...
        with get_db() as conn:
            ...
            if pin_to_store:
                ...
                if pin_row is not None:
                    from axiom.strategy_lifecycle import _extract_symbol_timeframe_from_config
...
def _try_research_recovery_on_edit(strategy_id: str):
    ...
    try:
        from axiom.db import get_db as _gdb, kv_get as _kvg, kv_set as _kvs
        ...
        debounce_key = f"axiom:recert_debounce:{strategy_id}"
        ...
        if result.get("promoted"):
            try:
                from axiom.api_domains.live_ws import ws_manager
                import asyncio
                try:
        ...
        import logging
        logging.getLogger("axiom.api_core").warning(
...
# â”€â”€ Phase 2: Axiom backtesting endpoints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def post_backtest_preview(body: BacktestPreviewBody):
    ...
    try:
        from axiom.strategies.backtest import preview_strategy_signals
...
def post_backtest_preview_chart(body: PreviewChartBody) -> dict:
    ...
    try:
        from axiom.strategies.backtest import build_strategy_preview_chart_context
...
async def post_nl_to_spec(body: NlToSpecBody) -> dict:
    ...
    from axiom.strategies.nl_spec_gen import nl_to_rule_spec
...
def register_manual_backtest_strategy(body: ManualStrategyBody) -> dict:
    ...
    try:
        from axiom.strategies.registry import _TYPE_MAP, discover, reset
        discover()
        if type_name in _TYPE_MAP:
            existing_module = str(getattr(_TYPE_MAP[type_name], "__module__", ""))
            our_module = f"axiom.strategies.custom.manual_{type_name}"
...
def send_manual_strategy_to_forge(body: SendToForgeBody) -> dict:
    ...
    """
    from axiom.strategies.registry import _TYPE_MAP, discover
    from axiom.db import create_strategy_container
    ...
    if mode == "visual":
        ...
        try:
            from axiom.strategies.builtin.rule_engine import validate_rule_spec
...
def _resolve_local_backtest_execution_params(
    ...
    *,
    ...
    from axiom.strategies.params import extract_execution_params_from_rule_blobs
...
def _backtest_task_title_prefix(request_source: str) -> str:
    ...
    if normalized in {"agent_tool", "AXIOM_agent_tool", "strategy_developer_tool"}:
...
def _agent_backtest_source(request_source: str) -> bool:
    ...
    normalized = str(request_source or "").strip().lower()
    return (
        normalized in {"agent_tool", "AXIOM_agent_tool", "strategy_developer_tool"}
...
def _create_inline_backtest_task(
    *,
    ...
    from axiom.db import get_db as _get_db, next_container_id
...
def _finalize_inline_backtest_task(
    *,
    ...
    if task_id is None:
        return
    from axiom.db import append_task_audit_event, get_db as _get_db
...
def post_backtesting_run(body: dict):
    ...
    try:
        ...
        if "strategy_id" in body and "dataset_id" in body:
            ...
            # Strip Axiom dataset prefix (e.g., "dataset-26-" from "dataset-26-BTC/USDT-1h")
        ...
        if is_remote:
            ...
            task_id = assign_task(
                ...
                description=(
                    ...
                    "Use AXIOM_list_datasets to find data, then AXIOM_create_strategy "
                    "and AXIOM_run_backtest to iterate."
                ),
                priority=10,
                source="user",
            )
