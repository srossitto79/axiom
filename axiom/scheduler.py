...
from axiom.model_routing import get_default_model_for_provider
from axiom.db import create_pending_task, get_db, init_db, is_user_active, kv_get, kv_set, kv_set_best_effort, log_activity, reap_long_running_agent_tasks, recover_stale_running_tasks
from axiom.system_pause import is_autonomy_paused, is_generation_paused
from axiom.task_timeouts import coerce_stale_recovery_minutes, recommended_agent_reaper_timeout_minutes, recommended_stale_recovery_minutes
...
_DEFAULT_JOB_IDS = {
    ...
    "Axiom-testing-cycle",
    "Axiom-paper-graduation",
    "Axiom-risk-audit",
    "Axiom-decay-tracker",
    "Axiom-weekly-review",
    "Axiom-regime-update",
    "Axiom-slippage-monitor",
    "Axiom-scanner-signal",
    "Axiom-scanner-hourly",
    "Axiom-recalibration",
    "Axiom-daily-learning",
    "Axiom-orphan-type-scan",
    "Axiom-decay-kill-switch",
    "Axiom-reconcile-sweep",
    "Axiom-source-reconciliation",
    "Axiom-market-data-collect",
    "Axiom-funding-history-reconcile",
    "Axiom-data-ohlcv-keepalive",
    "Axiom-data-oi-collect",
    "Axiom-data-funding-collect",
    "Axiom-data-engine-catchup",
    "Axiom-data-bv-backfill",
    "Axiom-overnight-summary",
    "Axiom-data-lsr-collect",
    "Axiom-data-taker-collect",
    "Axiom-data-liquidation-collect",
    "Axiom-data-fng-collect",
    "Axiom-data-macro-collect",
    "Axiom-data-btcdom-collect",
    "Axiom-quant-skills-consolidation",
    "Axiom-stale-triage",
    "Axiom-auto-intake",
    "Axiom-wal-checkpoint",
    "Axiom-db-maintenance",
    "Axiom-capital-slot-dedupe",
    "Axiom-hypothesis-verdict-loop",
    "Axiom-hypothesis-promotion-loop",
    "Axiom-hypothesis-revisit-pass",
    "Axiom-hypothesis-unstarted-ageout",
    "Axiom-gauntlet-step-loop",
    "Axiom-phantom-sweep",
    "Axiom-param-optimization",
}
...
_DATA_MANAGER_JOB_PAYLOAD_DEFAULTS: dict[str, dict[str, object]] = {
    ...
    "Axiom-data-oi-collect": {"kind": "data_manager_collect_oi", "timeout_seconds": 180},
    "Axiom-data-funding-collect": {"kind": "data_manager_collect_funding", "timeout_seconds": 180},
    "Axiom-data-lsr-collect": {"kind": "data_manager_collect_lsr", "timeout_seconds": 120},
    "Axiom-data-taker-collect": {"kind": "data_manager_collect_taker", "timeout_seconds": 120},
    "Axiom-data-liquidation-collect": {"kind": "data_manager_collect_liquidation", "timeout_seconds": 120},
    "Axiom-data-fng-collect": {"kind": "data_manager_collect_fng", "timeout_seconds": 120},
    "Axiom-data-macro-collect": {"kind": "data_manager_collect_macro", "timeout_seconds": 180},
    "Axiom-data-btcdom-collect": {"kind": "data_manager_collect_btcdom", "timeout_seconds": 120},
}
...
# (Axiom-scanner-signal / Axiom-scanner-hourly) are deliberately exempt:
...
def _job_has_live_background_task(job_id: str) -> bool:
    ...
    """
    name = f"Axiom-scheduler-job:{job_id}"
...
_CURRENT_SCHEDULER_JOB_ID: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "AXIOM_current_scheduler_job_id", default=None
)
...
def _defer_tracker_key(job_id: str) -> str:
    return f"axiom:scheduler:deferring:{job_id}"
...
def _apply_runtime_scheduler_overrides() -> int:
    ...
    overrides = {
        ...
        "Axiom-ideation-daily": ("interval", _runtime_interval_expr(int(tuning["ideation_interval_minutes"]))),
        "Axiom-testing-cycle": ("interval", _runtime_interval_expr(int(tuning["testing_interval_minutes"]))),
        "Axiom-paper-graduation": ("interval", _runtime_interval_expr(int(tuning["graduation_interval_minutes"]))),
        "Axiom-scanner-signal": ("interval", _runtime_interval_expr(int(tuning["scanner_signal_interval_minutes"]))),
        "Axiom-scanner-hourly": ("interval", _runtime_interval_expr(int(tuning["scanner_execution_interval_minutes"]))),
    }
    ...
    with get_db() as conn:
        for job_id, (schedule_type, schedule_expr) in overrides.items():
            ...
            payload_update = None
            if job_id == "Axiom-crucible-planner":
...
def migrate_legacy_scanner_cadence() -> bool:
    ...
    with get_db() as conn:
        ...
        conn.execute(
            ...
            ("Live Scanner Execution Worker", "300000", next_run, "Axiom-scanner-hourly"),
        )
    log.info("Migrated legacy scanner cadence from 60m to 5m (Axiom-scanner-hourly)")
    return True
...
def apply_startup_catchup(*, now: datetime | None = None) -> dict[str, int]:
    ...
    Axiom's lifecycle is "open the app → scheduler runs; close the app →
...
# still pass a tighter timeout. Override via AXIOM_DEFAULT_SYNC_JOB_TIMEOUT_SECONDS.
try:
    _DEFAULT_SYNC_JOB_TIMEOUT_SECONDS = float(
        os.environ.get("AXIOM_DEFAULT_SYNC_JOB_TIMEOUT_SECONDS", "600") or 600.0
    )
except (TypeError, ValueError):
...
async def _run_sync_job(fn, *args, timeout_seconds: float | None = None, **kwargs):
    ...
    executor = concurrent.futures.ThreadPoolExecutor(
        max_workers=1,
        thread_name_prefix="Axiom-scheduler-job",
    )
...
def _run_regime_update_job() -> int:
    from axiom.regime import TREND_DOWN, TREND_UP, RANGE_BOUND, HIGH_VOL, detect_regime
...
async def run_job(job: dict) -> tuple[str, str | None]:
    ...
    try:
        ...
        if kind == "scanner_signal_run":
            from axiom.scanner import run_signal_scan
            timeout_seconds = _coerce_timeout_seconds(
                payload.get("timeout_seconds"),
                _SCANNER_JOB_TIMEOUT_SECONDS,
            )
        ...
        if kind == "fitness_eval":
            from axiom.brain import run_strategy_review
        ...
        if kind == "daily_learning":
            from axiom.jobs.daily_learning import run_daily_learning
        ...
        if kind == "evolution_ideation":
            from axiom.evolution import run_ideation_step
            await _run_sync_job(run_ideation_step)
            return "ok", None

        if kind == "evolution_coding":
            from axiom.evolution import run_coding_step
            await _run_sync_job(run_coding_step)
            return "ok", None

        if kind == "evolution_testing":
            from axiom.evolution import run_testing_step
        ...
        if kind == "evolution_graduation":
            from axiom.evolution import check_paper_graduation
            await _run_sync_job(check_paper_graduation)
            return "ok", None

        if kind == "evolution_review":
            from axiom.evolution import run_weekly_review
        ...
        if kind == "param_optimization":
            from axiom.strategies.optimizer import optimize_all_deployed
            await _run_sync_job(optimize_all_deployed)
            return "ok", None

        # Risk audit — assign audit task to risk-manager
        if kind == "risk_audit":
            from axiom.brain import assign_risk_audit
        ...
        if kind == "decay_tracker":
            ...
            try:
                from axiom.monitoring import compute_paper_live_drift
                await _run_sync_job(compute_paper_live_drift)
            except Exception as exc:
                log.warning("decay_tracker: drift snapshot refresh failed: %s", exc)
            return "ok", None

        # Stale quick-screen triage — daily janitor (mirrors `Axiom strategies triage-stale`)
        ...
        if kind == "auto_intake":
            from axiom.strategies.intake import auto_intake_recent_files
        ...
        if kind == "wal_checkpoint":
            from axiom.db import checkpoint_wal
        ...
        if kind == "db_maintenance":
            from axiom.maintenance import run_db_maintenance
            raw_settings = kv_get("axiom:pipeline:settings", {}) or {}
            settings = raw_settings if isinstance(raw_settings, dict) else {}
        ...
        if kind == "capital_slot_dedupe":
            from axiom.policy import dedupe_capital_slots
            result = await _run_sync_job(
        ...
        if kind == "recalibrate":
            try:
                from axiom.recalibrator import check_and_recalibrate
        ...
        if kind == "ghost_container_scan":
            from axiom.db import run_daily_ghost_detection
        ...
        if kind == "decay_kill_switch":
            from axiom.monitoring import run_decay_kill_switch
        ...
        if kind == "reconcile_sweep":
            from axiom.scanner import sweep_pending_close_reconcile
        ...
        # that feeds the cache-only promotion gate (Axiom.policy). Never runs inside
        # a promotion write txn, so it is free to fetch the live venue here.
        if kind == "source_reconciliation":
            from axiom.source_reconciliation import run_source_reconciliation_job
            result = await _run_sync_job(
        ...
        if kind == "market_data_collect":
            from axiom.market_data_collector import collect_current_snapshot
        ...
        if kind == "funding_history_reconcile":
            from axiom.market_data_collector import DEFAULT_FUNDING_TARGET_DAYS, reconcile_funding_history
        ...
        if kind == "data_engine_catchup":
            ...
            from axiom.api_domains.data import execute_data_engine_catchup
        ...
        if kind == "phantom_sweep":
            from axiom.phantom_recovery import run_phantom_recovery_sweep
        ...
        if kind == "hypothesis_revisit_pass":
            from axiom.hypothesis_revisit import run_revisit_pass
        ...
        if kind == "overnight_summary":
            from axiom.jobs.overnight_summary import run_overnight_summary_job
...
async def tick():
    ...
    try:
        from axiom.gauntlet.engine import recover_stale_running_steps
    ...
    try:
        from axiom.control_plane.routines import DIRTY_FLAG_KEY as _ROUTINES_DIRTY_KEY
    ...
    try:
        from axiom.control_plane.approvals import expire_overdue_approvals
...
def _log_resolved_config_snapshot():
    ...
    try:
        settings = kv_get("axiom:settings", {})
        pipeline_thresholds = kv_get("axiom:pipeline_thresholds", {})
        pipeline_settings = kv_get("axiom:pipeline:settings", {})
        runtime_task_timeouts = _load_runtime_task_timeout_settings()

        snapshot = {
            "AXIOM_settings": settings if isinstance(settings, dict) else {},
    ...
    try:
        ...
        with get_db() as conn:
            scanner_job = conn.execute(
                "SELECT schedule_type, schedule_expr FROM scheduler_jobs WHERE id = 'Axiom-scanner-hourly'"
            ).fetchone()
...
async def run_scheduler_loop(interval_seconds: int = 30):
    ...
    # App-open catch-up: collapse missed cycles into a single run. Axiom
    ...
    while True:
        try:
            ...
            if consecutive_errors >= _SCHEDULER_CIRCUIT_BREAKER_TERMINATE:
                from axiom.config import is_beta_build
...
def seed_AXIOM_jobs():
    """Create the default Axiom 2.0 Continuous Learning scheduler jobs."""
    init_db()
    ...
    add_job(
        job_id="Axiom-crucible-discovery",
    ...
    add_job(
        job_id="Axiom-quant-skills-consolidation",
    ...
    add_job(
        job_id="Axiom-param-optimization",
    ...
    add_job(
        job_id="Axiom-capital-slot-dedupe",
    ...
    add_job(
        job_id="Axiom-decay-kill-switch",
    ...
    add_job(
        job_id="Axiom-source-reconciliation",
    ...
    add_job(
        job_id="Axiom-market-data-collect",
    ...
    add_job(
        job_id="Axiom-funding-history-reconcile",
    ...
    add_job(
        job_id="Axiom-data-ohlcv-keepalive",
    ...
    add_job(
        job_id="Axiom-data-funding-collect",
    ...
    add_job(
        job_id="Axiom-data-engine-catchup",
    ...
    add_job(
        job_id="Axiom-overnight-summary",
    ...
    add_job(
        job_id="Axiom-data-taker-collect",
    ...
    add_job(
        job_id="Axiom-data-liquidation-collect",
    ...
    add_job(
        job_id="Axiom-data-macro-collect",
    ...
    add_job(
        job_id="Axiom-data-btcdom-collect",
    ...
    with get_db() as conn:
        conn.execute(
            ...
            UPDATE scheduler_jobs
            SET enabled = 1
            WHERE id = 'Axiom-hypothesis-promotion-loop'
            """,
        )
    ...
    add_job(
        job_id="Axiom-hypothesis-unstarted-ageout",
    ...
    log.info("Seeded Axiom Continuous Learning jobs")


def reconcile_AXIOM_jobs() -> dict[str, int]:
    ...
    the Axiom rename. Leaving them enabled makes the same workflow run twice.
...
def ensure_monitoring_jobs() -> int:
    ...
    if "Axiom-decay-tracker" not in existing_ids:
        add_job(
            job_id="Axiom-decay-tracker",
            name="Strategy Decay Tracker",
            schedule_type="interval",
    ...
    if "Axiom-slippage-monitor" not in existing_ids:
        add_job(
            job_id="Axiom-slippage-monitor",
            name="Execution Slippage Monitor",
            schedule_type="interval",
        ...
        added += 1
    if "Axiom-recalibration" not in existing_ids:
        add_job(
            job_id="Axiom-recalibration",
            name="Adaptive Regime Recalibration",
            schedule_type="interval",
        ...
        added += 1
    if "Axiom-scanner-signal" not in existing_ids:
        tuning = _load_runtime_scheduler_tuning()
        add_job(
            job_id="Axiom-scanner-signal",
            name="Live Scanner Signal Worker",
...
def migrate_from_openclaw():
    """Import scheduler jobs from OpenClaw — but replace with Axiom equivalents."""
    ...
    console.print(f"\n[bold green]Created {len(jobs)} Axiom scheduler jobs[/bold green]")
