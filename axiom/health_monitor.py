...
from axiom.system_mode_policy import autonomous_runtime_allowed

log = logging.getLogger("axiom.health_monitor")
...
# Axiom:settings.staleness_thresholds.
...
def check_scheduler() -> ComponentStatus:
    ...
    try:
        ...
        from axiom.scheduler import get_enabled_jobs
        ...
        try:
            from axiom.scheduler import get_last_tick_at
...
def check_data_collector() -> ComponentStatus:
    ...
    try:
        ...
        jobs = get_enabled_jobs()
        data_job_ids = {
            "Axiom-data-ohlcv-keepalive", "Axiom-data-funding-collect",
            "Axiom-data-lsr-collect", "Axiom-data-taker-collect",
            "Axiom-data-liquidation-collect", "Axiom-data-fng-collect",
            "Axiom-data-macro-collect", "Axiom-data-btcdom-collect",
        }
...
def _stream_staleness_sla_minutes(stream: str) -> float:
    ...
    Operator-overridable via Axiom:settings.staleness_thresholds (minutes per
...
def check_lab_worker() -> ComponentStatus:
    ...
    try:
        ...
        from axiom.lab_worker_service import WORKER_STATUS_META_KEY
...
def _kill_lab_worker_processes() -> int:
    ...
    try:
        ...
        pid_file = AXIOM_HOME / "lab" / "lab_worker.pid"
        if pid_file.exists():
...
async def _attempt_recovery(state: HealthState, name: str, status: ComponentStatus) -> None:
    ...
    try:
        if name == "data_collector":
            ...
            from axiom.lab_worker_service import _reconcile_orchestrator_state
...
def check_ai_providers() -> ComponentStatus:
    ...
    """
    try:
        from axiom.provider_runtime_health import get_provider_health_runtime
...
def check_pipeline_throughput() -> ComponentStatus:
    ...
    """
    try:
        from axiom.lab_db import get_lab_meta, list_lab_jobs, LabJobState
        from axiom.lab_worker_service import PIPELINE_PROGRESS_META_KEY, WORKER_STATUS_META_KEY
