...
from axiom.db import get_db, kv_get, kv_set, kv_set_best_effort, log_activity, next_container_id
from axiom.sim.clock import get_now, get_today, sim_kv_key
from axiom.system_pause import is_system_paused
...
log = logging.getLogger("axiom.exchange.risk")
    ...
    *,
    ...
    with _POSITION_LOCK:
        ...
        if risk_pct > max_risk_per_trade:
            return False, 0.0, f"Risk {risk_pct:.1%} exceeds per-trade max {max_risk_per_trade:.1%}. Needs operator approval."
        ...
        if mode == "live":
            try:
                ...
                try:
                    from axiom.exchange import books as _books
                ...
                if acct_val > 0:
                    ...
                    _books_on = False
                    try:
                        from axiom.exchange import books as _books_mod
...
def cancel_reduce_only_orders_for_asset(
    ...
    *,
    ...
    if open_orders is None:
        try:
            from axiom.exchange.hyperliquid import get_open_orders
...
def _repair_position_protection(
    ...
    *,
    ...
    try:
        from axiom.exchange.hyperliquid import place_protective_stop
...
def _insert_recovered_trade(
    ...
    *,
    ...
    from axiom.config import get_execution_mode as _get_execution_mode
...
def _recover_exit_from_fills(
    ...
    *,
    ...
    try:
        from axiom.exchange.hyperliquid import get_user_fills
...
def reset_kill_switch():
    """Manually reset the kill-switch after review. Only the operator can do this.
