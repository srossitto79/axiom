...
from axiom.db import get_open_trades, get_recent_trades, get_strategies, kv_get, list_approvals
from axiom.strategy_diversity import filter_recall_records_for_diversity, render_strategy_diversity_guard
...
def _format_brain_lessons(limit: int = 10, min_confidence: float = 0.5) -> str:
    ...
    """
    try:
        from axiom.brain_lessons import list_lessons
...
async def store_conversation(
    ...
    Args:
        ...
        source: Conversation origin. Defaults to ``'ui_chat'`` for the in-app
            ...
            ``Axiom/bot.py`` caller keeps working without an edit.
    """
    try:
        ...
        summary = f"{prefix} Operator: {user_msg[:200]} | Axiom: {ai_response[:300]}"
...
def _get_learned_skills_context() -> str:
    ...
    """
    try:
        from axiom.quant_skills import get_ideation_context

        regime: str | None = None
        try:
            from axiom.regime import TRACKED_ASSETS, peek_cached_regime
...
def get_brain_learning_injection(query: str = "", *, n_results: int = 6, n_lessons: int = 8) -> str:
    ...
    try:
        from axiom import brain_lessons
