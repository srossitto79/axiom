...
from axiom.research_sources.url_ingest import fetch_preview
from axiom.system_mode_policy import is_manual_mode
...
def _compute_signals_safely(hypothesis_id: str) -> dict[str, Any] | None:
    ...
    try:
        from axiom.hypothesis_verdict import compute_verdict_signals
...
def get_ranked_data_gap_payload(limit: int = 20) -> dict[str, Any]:
    ...
    if items:
        from axiom.db import get_data_gap_requesters
...
def _enqueue_generate_strategies(
    *,
    ...
    description = (
        ...
        f"lane={hypothesis.get('lane') or 'benchmarking'}.\n"
        f"   - TEMPLATE path: use create_strategy when a built-in family fits (RSI, MACD, "
        f"Bollinger, EMA cross, etc.) and the mechanism is OHLCV-only.\n"
        f"   - CUSTOM CODE path: use create_custom_strategy when the mechanism needs "
        f"non-OHLCV columns (enrichment metrics like liquidations, L2 order book, funding, "
        f"open interest, long/short ratio, on-chain/social) or logic no template family "
        f"supports. The exact columns and their date ranges for this hypothesis's target "
        f"assets/timeframes are in the DATA AVAILABILITY section of your task context. "
...
def _enqueue_operator_manual_research(
    *,
    ...
    description = (
        ...
        f"3. Spawn 1-3 candidate strategies for hypothesis_id={hypothesis['id']}, "
        f"lane=benchmarking.\n"
        f"   - TEMPLATE path: use create_strategy when a built-in family fits (RSI, MACD, "
        f"Bollinger, EMA cross, etc.) and the mechanism is OHLCV-only.\n"
        f"   - CUSTOM CODE path: use create_custom_strategy when the mechanism needs "
...
def _enqueue_operator_seed_research(
    *,
    ...
    description = (
        ...
        f"4. Spawn 1-3 candidate strategies for hypothesis_id={hypothesis['id']}, "
        f"lane=benchmarking.\n"
        f"   - TEMPLATE path: use create_strategy when a built-in family fits (RSI, MACD, "
        f"Bollinger, EMA cross, etc.) and the mechanism is OHLCV-only.\n"
        f"   - CUSTOM CODE path: use create_custom_strategy when the mechanism needs "
