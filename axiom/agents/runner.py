...
from axiom.db import claim_pending_agent_tasks, create_pending_task, format_prefixed_id, get_db, get_task_tool_calls, init_db, is_user_active, kv_get, kv_set, log_activity
from axiom.provider_runtime_health import (
    record_call_failure,
    record_provider_event,
    record_provider_ok,
)
from axiom.research_context import build_research_context, coerce_research_contract
from axiom.task_timeouts import DEFAULT_AGENT_TASK_TIMEOUT_SECONDS, resolve_agent_task_timeout_seconds
...
def _resolve_tool_call_chain(
    ...
    if agent_id:
        try:
            from axiom.model_selection import _policy_slot_fallbacks
...
def _provider_has_credentials(provider: str) -> bool:
    ...
    """
    try:
        from axiom.ai import get_token
        get_token(provider)
        return True
    except Exception:
        return False
...
def _first_configured_provider() -> tuple[str, str] | None:
    ...
    """
    try:
        from axiom.model_routing import get_default_model_for_provider, get_model_routing
...
def _resolve_backup_provider(primary_provider: str) -> tuple[str, str] | None:
    ...
    """
    try:
        from axiom.config import get_backup_ai_model, get_backup_ai_provider
...
async def _call_with_tools(
    ...
    # the manual CLI `Axiom brain-invoke` path calls call_ai() directly and is not
    ...
    if not _provider_has_credentials(primary_provider):
        ...
        if backup is not None:
            ...
            from axiom.auth.store import CredentialError, credential_status
...
async def _call_with_tools_single(
    ...
    for round_num in range(MAX_TOOL_ROUNDS):
        ...
        if round_num > 0:
            from axiom.billing_guard import check_daily_cost_cap
        ...
        from axiom.model_selection import assert_callable
...
def _error_detail(error: Exception, body_limit: int = 4000) -> str:
    ...
    SECRET-REDACTED at the source (Axiom.redact): this string is persisted to
...
def _emit_provider_quota_alert(provider: str, detail: str) -> None:
    ...
    cooldown_key = f"axiom:provider-quota-alert:{provider_key}"
...
def _research_task_already_performed_first_wave(task_display_id: str) -> bool:
    ...
    attempted_follow_through = any(
        ...
        in {"AXIOM_create_strategy", "register_strategy", "run_backtest", "AXIOM_run_backtest"}
        for call in tool_calls
    )
...
def _list_hypothesis_strategies_for_follow_through(hypothesis_id: str) -> list[dict]:
    from axiom.hypotheses import list_hypothesis_strategies
    ...
    *,
    ...
    for _, hypothesis in candidates:
        ...
        description = (
            ...
            "1. Use AXIOM_create_strategy or register_strategy to create a linked strategy container.\n"
...
async def run_agent_task(agent: dict, task: dict) -> dict:
    ...
    if not can_run:
        if task_type == "phantom_repair" and strategy_id:
            from axiom.phantom_recovery import mark_phantom_recovery_exhausted
    ...
    if task_type == "phantom_repair" and strategy_id:
        from axiom.phantom_recovery import mark_phantom_recovery_repair_running
    ...
    try:
        ...
        if not is_trade_execution_task:
            ...
            if _requeue_agent_task(
                ...
                if task_type == "phantom_repair" and strategy_id:
                    from axiom.phantom_recovery import mark_phantom_recovery_repair_pending
...
async def _run_agent_task_inner(
    ...
    try:
        ...
        if is_trade_execution_task:
            from axiom.scanner import execute_trade_intent
        ...
        from axiom.redact import redact_dict
        ...
        if task_type == "phantom_repair" and strategy_id:
            from axiom.phantom_recovery import handle_phantom_repair_completion
        ...
        if strategy_id:
            try:
                from axiom.brain import NEXT_STAGE, STAGE_TO_AGENT, transition_stage
        ...
        with get_db() as conn:
            ...
            if handoff_target_agent:
                from axiom.db import handoff_task
        ...
        try:
            from axiom.reporter import broadcast_agent_task
        ...
        from axiom.model_selection import UnconfiguredRouteError
