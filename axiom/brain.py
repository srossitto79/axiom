...
from axiom.context import build_brain_context
...
from axiom.hypotheses import require_hypothesis
...
try:
    from axiom.policy import verify_backtest_exists_for_stage_transition
except ImportError:
...
log = logging.getLogger("axiom.brain")
...
def _gauntlet_entry_guardrails(strategy_id: str, metrics: dict) -> tuple[bool, str]:
    ...
    from axiom.metrics_integrity import check_metrics_integrity, data_quality_hold_reason
...
def _auto_approve_promotions_enabled(
    ...
    try:
        ...
        pipeline_settings = kv_get("axiom:pipeline:settings", {}) or {}
...
def transition_stage(
    ...
    with get_db() as conn:
        ...
        if normalized_target == "paper":
            from axiom.policy import _paper_slot_competition_enabled
        ...
        if normalized_target in ("quick_screen", "gauntlet"):
            try:
                from axiom.gauntlet.store import WORKFLOW_DEFINITION_VERSION
    ...
    # transition itself.
    try:
        from axiom.brain_decisions import backfill_outcome_for_strategy
    ...
    if not skip_outcome_closure:
        ...
        if outcome_kind is not None:
            try:
                from axiom.skill_outcomes import record_outcome as _record_outcome
...
def execute_brain_actions(
    ...
    *,
    ...
    """
    from axiom.brain_decisions import (
        link_agent_task,
        record_decision,
        update_action_taken,
    )
...
async def invoke(
    ...
    try:
        from axiom.context import get_brain_learning_injection
    ...
    # (P1-T05).
    if message:
        from axiom.sanitize import sanitize_operator_input
    ...
    from axiom.brain_inject import (
        build_user_message,
        compute_prompt_hash,
...
def _build_cycle_prompt() -> str:
    ...
    return (
        "You are the Brain ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ the boss of the Axiom trading operation.\n\n"
...
def run_strategy_review():
    ...
    """
    from axiom.strategies.fitness import get_promotion_candidates
...
def process_task_queue():
    ...
    from axiom.db import claim_pending_tasks
    ...
    for task in rows:
        ...
        try:
            ...
            if channel:
                try:
                    from axiom.notifications import emit_notification
                    from axiom.notification_renderers import summarize_discord_text
...
def run_gauntlet_backtest_migration():
    ...
    Guarded by KV flag ``Axiom:migration:gauntlet_backtest_demotion_done`` so it runs once.
    Snapshots each affected strategy before demotion.
    """
    from axiom.db import save_migration_snapshot

    flag_key = "axiom:migration:gauntlet_backtest_demotion_done"
    if kv_get(flag_key):
        return
